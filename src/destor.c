...
//分块算法
	destor.chunk_algorithm = CHUNK_RABIN;//Rabin指纹
	destor.chunk_max_size = 65536;
	destor.chunk_min_size = 1024;
	destor.chunk_avg_size = 8192;
//cache访问方式及大小
	destor.restore_cache[0] = RESTORE_CACHE_LRU;//默认为LRU算法
...
//分段算法，固定大小分段，均匀采样
	...
	if ((fp = fopen(stat_file, "r"))) {
		...
		fread(&last_retention_time, 4, 1, fp);//retention,保留
...
int main(int argc, char **argv) {
	...
	while ((opt = getopt_long(argc, argv, short_options, long_options, NULL))
		...
		case 'r':
			job = DESTOR_RESTORE;
			revision = atoi(optarg);//atoi()将字符串转换为长整数
			break;
		case 's':
			destor_stat();//输出系统存储状态信息
			break;
		case 't':
	...
	case DESTOR_RESTORE:
		...
		do_restore(revision, path[0] == 0 ? 0 : path);//恢复入口

...
void append_file_recipe_meta(struct backupVersion* b, struct fileRecipeMeta* r) {
	...
	if(sizeof(len) + len + sizeof(r->chunknum) + sizeof(r->filesize)+sizeof(r->row)+sizeof(r->column)> metabufsize - metabufoff){
		/* buf is full */
		fwrite(metabuf, metabufoff, 1, b->metadata_fp);
		metabufoff = 0;
	}
	...
	memcpy(metabuf + metabufoff, &r->row, sizeof(r->row));
	metabufoff += sizeof(r->row);
	memcpy(metabuf + metabufoff, &r->column, sizeof(r->column));
	metabufoff += sizeof(r->column);
...
segmentid append_segment_flag(struct backupVersion* b, int flag, int segment_size){
	...
	if(flag == CHUNK_SEGMENT_START){
		...
		segmentlen = (sizeof(fingerprint) + sizeof(containerid) + sizeof(int32_t)+sizeof(int)*2)*2
				+ segment_size * (sizeof(fingerprint) + sizeof(containerid) + sizeof(int32_t)+sizeof(int)*2);
	...
	memcpy(segmentbuf + segmentbufoff, &cp.row, sizeof(int));
	segmentbufoff += sizeof(int);
	memcpy(segmentbuf + segmentbufoff, &cp.column, sizeof(int));
	segmentbufoff += sizeof(int);
...
void append_n_chunk_pointers(struct backupVersion* b,
	...
	for (i = 0; i < n; i++) {
		...
		segmentbufoff += sizeof(int32_t);
		memcpy(segmentbuf + segmentbufoff, &(bcp.row), sizeof(int));
		segmentbufoff += sizeof(int);
		memcpy(segmentbuf + segmentbufoff, &(bcp.column), sizeof(int));
		segmentbufoff += sizeof(int);
...
struct fileRecipeMeta* read_next_file_recipe_meta(struct backupVersion* b) {
	...
	fread(&r->row,sizeof(r->row),1,b->metadata_fp);
	fread(&r->column,sizeof(r->column),1,b->metadata_fp);
...
struct chunkPointer* read_next_n_chunk_pointers(struct backupVersion* b, int n,
	...
	for (i = 0; i < num; i++) {
		...
		fread(&(cp[i].row),sizeof(int),1,b->recipe_fp);
		fread(&(cp[i].column),sizeof(int),1,b->recipe_fp);
...
GQueue* prefetch_segments(segmentid id, int prefetch_num) {
	...
	for (j = 0; j < prefetch_num; j++) {
		...
		fread(&flag.row, sizeof(flag.row), 1, opened_bv->recipe_fp);
		fread(&flag.column, sizeof(flag.column), 1, opened_bv->recipe_fp);
		...
		for (i = 0; i < flag.size; i++) {
			...
			fread(&cp->row, sizeof(cp->row), 1, opened_bv->recipe_fp);
			fread(&cp->column, sizeof(cp->column), 1, opened_bv->recipe_fp);
...
struct segmentRecipe* read_next_segment(struct backupVersion *bv){
	...
	ret += fread(&flag.row, sizeof(flag.row), 1, bv->recipe_fp);
	ret += fread(&flag.column, sizeof(flag.column), 1, bv->recipe_fp);
	if(ret != 5 || flag.id != -CHUNK_SEGMENT_START){
	...
	for (i = 0; i < flag.size; i++) {
		...
		fread(&cp->row, sizeof(cp->row), 1, bv->recipe_fp);
		fread(&cp->column, sizeof(cp->column), 1, bv->recipe_fp);
		if(cp->id <= TEMPORARY_ID){
			WARNING("expect > 0, but being %lld", cp->id);
			assert(cp->id > TEMPORARY_ID);
		}
	...
	fread(&flag.row, sizeof(flag.row), 1, bv->recipe_fp);
	fread(&flag.column, sizeof(flag.column), 1, bv->recipe_fp);
	assert(flag.id == 0 - CHUNK_SEGMENT_END);
